...
<%
	...
	int searchCount = anonymousDAO.getSearchCount(search);
	...
	List<AnonymousBean> list = null;
	
	if(searchCount > 0){		//DB에 글이 존재하면
		//글목록 검색해서 가져오기
		//getBoardList(각 페이지마다 첫번째로 보여질 시작 글번호, 한페이지당 보여줄 글 개수)
		list = anonymousDAO.getSearchList(startRow, pageSize, search);
	}
		...
		<div id="page_control">
			<%
				if(searchCount > 0){
					...
					if(currentPage>1){
			%>
						<a href = "search.jsp?search=<%=search%>&search_btn=글검색&pageNum=<%=currentPage-1%>">[이전]</a>
			<%
					}
					//[1][2]... 페이지 번호 나타내기
			...
			%>
						<a id= "a<%=i%>" href = "search.jsp?search=<%=search%>&search_btn=글검색&pageNum=<%=i%>">[<%=i %>]</a>
			<%
					}//안쪽 for문
					//[다음] 끝페이지번호가 전체 페이지 개수보다 작을 때
					if(currentPage<pageCount){
			%>
						<a href="search.jsp?search=<%=search%>&search_btn=글검색&pageNum=<%=currentPage+1%>">[다음]</a>
			<%
					}
				}//바깥 if문
			%>
