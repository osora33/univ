...
<%
	...
	int count = anonymousDAO.getAnonymousCount();
	...
	if(count > 0){		//DB에 글이 존재하면
		...
		list = anonymousDAO.getAnonymousList(startRow, pageSize);
	}
		...
		<div id="page_control">
			<%
				if(count > 0){
					...
					if(currentPage>1){
			%>
						<a href = "anonymous.jsp?pageNum=<%=currentPage-1%>">[이전]</a>
			<%
					}
					//[1][2]... 페이지 번호 나타내기
			...
			%>
						<a id= "a<%=i%>" href = "anonymous.jsp?pageNum=<%=i%>">[<%=i %>]</a>
			<%
					}//안쪽 for문
					//[다음] 끝페이지번호가 전체 페이지 개수보다 작을 때
					if(currentPage<pageCount){
			%>
						<a href="anonymous.jsp?pageNum=<%=currentPage+1%>">[다음]</a>
			<%
					}
				}//바깥 if문
			%>
