...
<%
	...
	//DB작업할 객체 생성
	EctDAO ectDAO = new EctDAO();

	//DB에 저장된 전체 글 개수 검색해오는 메소드 호출
	int searchCount = ectDAO.getSearchCount(frselect, search);
	...
	List<EctBean> list = null;
	
	if(searchCount > 0){		//DB에 글이 존재하면
		//글목록 검색해서 가져오기
		//getBoardList(각 페이지마다 첫번째로 보여질 시작 글번호, 한페이지당 보여줄 글 개수)
		list = ectDAO.getSearchList(startRow, pageSize, frselect, search);
	}
...
<%-- 						<a href = "freeboard.jsp?pageNum=<%=startPage-pageBlock %>">[이전]</a> --%>
						<a href = "search.jsp?frselect=<%=frselect%>&search=<%=search%>&search_btn=글검색&pageNum=<%=currentPage-1%>">[이전]</a>
			<%
					}
					//[1][2]... 페이지 번호 나타내기
			...
			%>
						<a id= "a<%=i%>" href = "search.jsp?frselect=<%=frselect%>&search=<%=search%>&search_btn=글검색&pageNum=<%=i%>">[<%=i %>]</a>
			<%
					}//안쪽 for문
...
<%-- 						<a href="freeboard.jsp?pageNum=<%=startPage+pageBlock%>">[다음]</a> --%>
						<a href="search.jsp?frselect=<%=frselect%>&search=<%=search%>&search_btn=글검색&pageNum=<%=currentPage+1%>">[다음]</a>
			<%
					}
				}//바깥 if문
			%>
