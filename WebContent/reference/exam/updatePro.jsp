			...
			try {
				...
				if (fileName == null) { // 파일이 업로드 되지 않았을때
					//3. Boardean객체를 생성하여 DB에 추가할 글 내용 저장
				     ...
				     exambean.setNum(num);
				     exambean.setContent(content);
				     exambean.setSubject(subject);
				     exambean.setName(name);
				     exambean.setDate(timestamp);
				     exambean.setIp(ip);
				    
				     //4. galaryDAO 객체를 생성하여 새로운 글 내용을 추가할 insertgalary 메소드 호출시 galaryBean 객체를 전달
				     ExamDAO dao = new ExamDAO();
				     dao.updateExam(exambean);
				     ...
				     exambean.setDate(timestamp);
				     exambean.setIp(ip);
				     exambean.setFile(fileName);
				    
				     //4. galaryDAO 객체를 생성하여 새로운 글 내용을 추가할 insertgalary 메소드 호출시 galaryBean 객체를 전달
				     ExamDAO dao = new ExamDAO();
				     dao.updateExamF(exambean);
