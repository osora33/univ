...
<%
	...
	int count = examDAO.getExamCount();
	...
	if(count > 0){		//DB에 글이 존재하면
		...
		list = examDAO.getExamList(startRow, pageSize);
	}
...
<%-- 						<a href = "freeboard.jsp?pageNum=<%=startPage-pageBlock %>">[이전]</a> --%>
						<a href = "exam.jsp?pageNum=<%=currentPage-1%>">[이전]</a>
			<%
					}
					//[1][2]... 페이지 번호 나타내기
			...
			%>
						<a id= "a<%=i%>" href = "exam.jsp?pageNum=<%=i%>">[<%=i %>]</a>
			<%
					}//안쪽 for문
...
<%-- 						<a href="freeboard.jsp?pageNum=<%=startPage+pageBlock%>">[다음]</a> --%>
						<a href="exam.jsp?pageNum=<%=currentPage+1%>">[다음]</a>
			<%
					}
				}//바깥 if문
			%>
