...
<%
	...
	//DB작업할 객체 생성
	BuyDAO galaryDAO = new BuyDAO();

	//DB에 저장된 전체 글 개수 검색해오는 메소드 호출
	int count = galaryDAO.getBuyCount();
	...
	if(count > 0){		//DB에 글이 존재하면
		...
		list = galaryDAO.getBuyList(startRow, pageSize);
	}
	...
	<article>
		<div id="wrapper2">
			...
			%>
							...
							<td class="price" width="25%"><%=bean.getPrice() %></td>
						</tr>
					</table>
			<%
					}
...
<%-- 						<a href = "freeboard.jsp?pageNum=<%=startPage-pageBlock %>">[이전]</a> --%>
					<a href = "buy.jsp?pageNum=<%=currentPage-1%>">[이전]</a>
		<%
				}
				//[1][2]... 페이지 번호 나타내기
		...
		%>
					<a id= "a<%=i%>" href = "buy.jsp?pageNum=<%=i%>">[<%=i %>]</a>
		<%
				}//안쪽 for문
...
<%-- 						<a href="freeboard.jsp?pageNum=<%=startPage+pageBlock%>">[다음]</a> --%>
					<a href="buy.jsp?pageNum=<%=currentPage+1%>">[다음]</a>
		<%
				}
			}//바깥 if문
		%>
