...
<%
	...
	String subncon = request.getParameter("subncon");
	String writer = request.getParameter("writer");
	String frselect = request.getParameter("frselect");
	
	//DB작업할 객체 생성
	BuyDAO buyDAO = new BuyDAO();

	//DB에 저장된 전체 글 개수 검색해오는 메소드 호출
	int searchCount = buyDAO.getSearchCount(frselect, search);
	...
	List<BuyBean> list = null;
	
	if(searchCount > 0){		//DB에 글이 존재하면
		//글목록 검색해서 가져오기
		//getBoardList(각 페이지마다 첫번째로 보여질 시작 글번호, 한페이지당 보여줄 글 개수)
		list = buyDAO.getSearchList(startRow, pageSize, frselect, search);
	}
...
<body>
		...
		<ul>
			...
			<li id="upper2" class="upper"><a href="../../market/sell/sell.jsp">장터</a>
				<ul id="lower2" class="lower">
					...
					<li><a href="../../market/buy/buy.jsp" class="now">삽니다</a></li>
				</ul>
			</li>
	...
	<article>
		<div id="wrapper2">
			<% 
				if(searchCount >0){		//DB에 글이 존재할 경우
					for(int i = 0; i<list.size(); i++){
						BuyBean bean = list.get(i);
			%>
					<table class="gtable">
			...
			%>
						...
						<tr class="post"  onclick = "location.href='content.jsp?num=<%=bean.getNum()%>&pageNum=<%=pageNum%>'" style="cursor: pointer;">
							<td class="postnum"width="10%"><%=bean.getNum()%>.</td>
							<td class="subject" width="40%"><%=bean.getSubject()%>
			<%
			...
			%>
							<td class="status" width="25%"><%=bean.getStatus() %></td>
							<td class="price" width="25%"><%=bean.getPrice() %>
						</tr>
					</table>
			<%
					}
			...
			%>
					<table class="ntable">
						<tr>
							<td class="thumnail" colspan="4">"<%=search%>"에 해당하는 글이 존재하지 않습니다</td>
						</tr>
					</table>
			<%
				}
		...
		<div id="undermenu">
				...
				<form action="search.jsp" method="get" name="fr">
					<div id="table_search">
						...
						<input type="text" name="search" id="search" class="input_box" value="<%=search %>"> 
...
<%-- 						<a href = "freeboard.jsp?pageNum=<%=startPage-pageBlock %>">[이전]</a> --%>
						<a href = "freeboard.jsp?pageNum=<%=currentPage-1%>">[이전]</a>
			<%
					}
					//[1][2]... 페이지 번호 나타내기
			...
			%>
						<a id= "a<%=i%>" href = "freeboard.jsp?pageNum=<%=i%>">[<%=i %>]</a>
			<%
					}//안쪽 for문
...
<%-- 						<a href="freeboard.jsp?pageNum=<%=startPage+pageBlock%>">[다음]</a> --%>
						<a href="freeboard.jsp?pageNum=<%=currentPage+1%>">[다음]</a>
			<%
					}
				}//바깥 if문
			%>
