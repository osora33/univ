...
<%
    ...
    buybean.setContent(content);
    buybean.setSubject(subject);
    buybean.setName(name);
    buybean.setDate(timestamp);
    buybean.setIp(ip);
    buybean.setImgPath(imgPath);
    buybean.setPrice(price);
    buybean.setStatus(status);
    
    //4. galaryDAO 객체를 생성하여 새로운 글 내용을 추가할 insertgalary 메소드 호출시 galaryBean 객체를 전달
    BuyDAO dao = new BuyDAO();
    dao.insertBuy(buybean);
