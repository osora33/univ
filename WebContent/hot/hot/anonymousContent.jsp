...
<%
	...
	//Anonymous 위한 DAO객체 생성
	AnonymousDAO dao = new AnonymousDAO();
	
	//DB에 존재하는 글의 조회수 1 증가시키기 위해 메소드 호출
	dao.updateReadCount(originNum);
	
	//글번호를 매개변수로 전달하여 글번호에 해당되는 글의 Anonymous색하기
	AnonymousBean anonymousBean = dao.getAnonymous(originNum);
	
	int DBnum = anonymousBean.getNum();		//검색한 글번호
	int DBReadcount = anonymousBean.getReadcount();		//검색한 글의 조회수
	String DBName = anonymousBean.getName();		//검색한 글의 작성자명
	Timestamp DBDate = anonymousBean.getDate();		//검색한 글 작성일
	String DBSubject = anonymousBean.getSubject();		//검색한 글 제목
	String DBContent = "";		//검색한 글 내용
	
	//검색한 글 내용이 존재한다면 엔터키를 사용해서 처리한 글 내용을 받아온다
	if(anonymousBean.getContent() != null){
		DBContent = anonymousBean.getContent().replace("\r\n", "<br>");
	}
	//답변글 처리 관련 검색한 필드의 정보 얻기
	int DBRe_ref = anonymousBean.getRe_ref();		//주 글의 그룹번호
	int DBRe_lev = anonymousBean.getRe_lev();		//주 글의 들여쓰기 정도값
	int DBRe_seq = anonymousBean.getRe_seq();		//주 글들 내의 순서값
	
	SimpleDateFormat sdf = new SimpleDateFormat("yyyy.MM.dd HH:mm:ss");
	
	//-------------------------------덧글 DB작업-------------------------------------
	ARippleDAO rippledao = new ARippleDAO();
	ARippleBean ripplebean = new ARippleBean();
	rippledao.getRipple(originNum);
...
// 	int rnum = ripplebean.getNum();
	...
	int count = rippledao.getRippleCount(originNum);
	...
	List<ARippleBean> list = null;
	
	if(count > 0){		//DB에 글이 존재하면
		//덧글목록 검색해서 가져오기
		//getAnonymousList(각 페이지마다 첫번째로 보여질 시작 글번호, 한페이지당 보여줄 글 개수)
		list = rippledao.getRippleList(startRow, pageSize, originNum);
	}
%>
	...
	<div id="wrap">
		...
		<article>
			<table id="notice">
				<tr>
					...
					<td><%=DBnum %></td>
					<td>작성일</td>
					<td width="30%"><%=sdf.format(DBDate) %></td>
					<Td>조회수</Td>
					<td><%=DBReadcount %></td>
				</tr>
				<tr>
					<td>글제목</td>
					<td colspan = "5"><%=DBSubject %></td>
				</tr>
				<tr class="contenttd">
					<td>글내용</td>
					<td colspan = "5" id="dbcontent"><%=DBContent %></td>
				</tr>
			</table>
			<%
				if(id != null){
			%>
			<form class = "rp" action="../../board/anonymous/ripple.jsp" method="post">
				<input type ="hidden" name="postnum" value="<%=originNum %>">
				<table>
					<tr>
			...
			if(count >0){		//DB에 글이 존재할 경우
				...
				for(int i = 0; i<list.size(); i++){
					ARippleBean bean = list.get(i);
			%>
				<form action="../../board/anonymous/rripple.jsp" method="post" name="fr">
					<tr>
					<%		
			...
			%>
								...
								<script>
									function rdelete(a){
										...
										if(rdelete){
											location.href = "../../board/anonymous/rdelete.jsp?pageNum=<%=pageNum%>&postnum=<%=bean.getPostnum()%>&num="+a;
										}
									}
								</script>
			<%
			...
			%>
					...
					<form action="../../board/anonymous/rupdate.jsp" method="post">
			...
			%>
				<input type="button" value="글목록" class="btn" onclick = "location.href='hot.jsp?pageNum=<%=pageNum %>'">
			</div>
			<div class="clear"></div>
		</article>
		<script>
			function update(){
				...
				if(update){
					location.href = "../../board/anonymous/update.jsp?num=<%=DBnum%>&pageNum=<%=pageNum%>";
				}
			}
			...
			function cdelete(){
				...
				if(cdelete){
					location.href="../../board/anonymous/deletePro.jsp?num=<%=DBnum %>&pageNum=<%=pageNum %>"
				}
			}
		</script>
		<!-- 게시판 -->
		...
		<div id="page_control">
			...
			if(count > 0){
				...
				if(currentPage>1){
		%>
					<a href = "anonymousContent.jsp?originNum=<%=originNum %>&rpageNum=<%=currentPage-1%>&pageNum=<%=pageNum%>">[이전]</a>
		<%
				}
				//[1][2]... 페이지 번호 나타내기
		...
		%>
					<a id= "a<%=i%>" href = "anonymousContent.jsp?originNum=<%=originNum %>&rpageNum=<%=i%>&pageNum=<%=pageNum%>">[<%=i %>]</a>
		<%
				}//안쪽 for문
				//[다음] 끝페이지번호가 전체 페이지 개수보다 작을 때
				if(currentPage<pageCount){
		%>
					<a href="anonymousContent.jsp?originNum=<%=originNum %>&rpageNum=<%=currentPage+1%>&pageNum=<%=pageNum%>">[다음]</a>
		<%
				}
			}//바깥 if문
			%>
