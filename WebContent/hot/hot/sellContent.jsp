...
<%
	...
	SellBean sellBean = dao.getSell(originNum);
	
	int DBnum = sellBean.getNum();		//검색한 글번호
	int DBReadcount = sellBean.getReadcount();		//검색한 글의 조회수
	String DBName = sellBean.getName();		//검색한 글의 작성자명
	Timestamp DBDate = sellBean.getDate();		//검색한 글 작성일
	...
	SRippleBean ripplebean = new SRippleBean();
	...
	<div id="wrap">
		...
		<article>
			...
			<%
				if(id != null){
			%>
			<form class = "rp" action="../../market/sell/ripple.jsp" method="post">
			...
			if(count >0){		//DB에 글이 존재할 경우
				...
				for(int i = 0; i<list.size(); i++){
					SRippleBean bean = list.get(i);
// 					int rnum = bean.getNum();
			%>
				<form action="../../market/sell/rripple.jsp" method="post" name="fr">
					<tr>
					<%		
			...
			%>
								...
								<script>
									function rdelete(a){
										...
										if(rdelete){
											location.href = "../../market/sell/rdelete.jsp?pageNum=<%=pageNum%>&postnum=<%=bean.getPostnum()%>&num="+a;
										}
									}
								</script>
			<%
...
<%-- 							<a href="rripple.jsp?postnum=<%=num%>&num=<%=bean.getNum()%>"> --%>
					...
					<form action="../../market/sell/rupdate.jsp" method="post">
		...
		<script>
			function update(){
				...
				if(update){
					location.href = "../../market/sell/update.jsp?num=<%=DBnum%>&pageNum=<%=pageNum%>";
				}
			}
			...
			function cdelete(){
				...
				if(cdelete){
					location.href="../../market/sell/deletePro.jsp?num=<%=DBnum %>&pageNum=<%=pageNum %>"
				}
			}
		</script>
		<!-- 게시판 -->
		...
		<div id="page_control">
			...
			if(count > 0){
				...
				if(currentPage>1){
		%>
					<a href = "sellContent.jsp?originNum=<%=originNum %>&rpageNum=<%=currentPage-1%>&pageNum=<%=pageNum%>">[이전]</a>
		<%
				}
				//[1][2]... 페이지 번호 나타내기
		...
		%>
					<a id= "a<%=i%>" href = "sellContent.jsp?originNum=<%=originNum %>&rpageNum=<%=i%>&pageNum=<%=pageNum%>">[<%=i %>]</a>
		<%
				}//안쪽 for문
				//[다음] 끝페이지번호가 전체 페이지 개수보다 작을 때
				if(currentPage<pageCount){
		%>
					<a href="sellContent.jsp?originNum=<%=originNum %>&rpageNum=<%=currentPage+1%>&pageNum=<%=pageNum%>">[다음]</a>
		<%
				}
			}//바깥 if문
			%>
