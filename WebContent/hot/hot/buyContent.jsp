...
<%
	...
	BuyBean buyBean = dao.getBuy(originNum);
	
	int DBnum = buyBean.getNum();		//검색한 글번호
	int DBReadcount = buyBean.getReadcount();		//검색한 글의 조회수
	String DBName = buyBean.getName();		//검색한 글의 작성자명
	Timestamp DBDate = buyBean.getDate();		//검색한 글 작성일
	String DBSubject = buyBean.getSubject();		//검색한 글 제목
	String DBContent = "";		//검색한 글 내용
	String DBStatus = buyBean.getStatus();
	
	//검색한 글 내용이 존재한다면 엔터키를 사용해서 처리한 글 내용을 받아온다
	if(buyBean.getContent() != null){
		DBContent = buyBean.getContent().replace("\r\n", "<br>");
	}
	SimpleDateFormat sdf = new SimpleDateFormat("yyyy.MM.dd HH:mm:ss");
	
	//-------------------------------덧글 DB작업-------------------------------------
	BRippleDAO rippledao = new BRippleDAO();
	...
	<div id="wrap">
		...
		<article>
			...
			<%
				if(id != null){
			%>
			<form class = "rp" action="../../market/buy/ripple.jsp" method="post">
			...
			if(count >0){		//DB에 글이 존재할 경우
				...
				for(int i = 0; i<list.size(); i++){
					BRippleBean bean = list.get(i);
// 					int rnum = bean.getNum();
			%>
				<form action="../../market/buy/rripple.jsp" method="post" name="fr">
					<tr>
					<%		
			...
			%>
								...
								<script>
									function rdelete(a){
										...
										if(rdelete){
											location.href = "../../market/buy/rdelete.jsp?pageNum=<%=pageNum%>&postnum=<%=bean.getPostnum()%>&num="+a;
										}
									}
								</script>
			<%
...
<%-- 							<a href="rripple.jsp?postnum=<%=num%>&num=<%=bean.getNum()%>"> --%>
					...
					<form action="../../market/buy/rupdate.jsp" method="post">
		...
		<script>
			function update(){
				...
				if(update){
					location.href = "../../market/buy/update.jsp?num=<%=DBnum%>&pageNum=<%=pageNum%>";
				}
			}
			...
			function cdelete(){
				...
				if(cdelete){
					location.href="../../market/buy/deletePro.jsp?num=<%=DBnum %>&pageNum=<%=pageNum %>"
				}
			}
		</script>
		<!-- 게시판 -->
...
<%-- 						<a href = "freeboard.jsp?pageNum=<%=startPage-pageBlock %>">[이전]</a> --%>
					<a href = "buyContent.jsp?originNum=<%=originNum %>&rpageNum=<%=currentPage-1%>&pageNum=<%=pageNum%>">[이전]</a>
		<%
				}
				//[1][2]... 페이지 번호 나타내기
		...
		%>
					<a id= "a<%=i%>" href = "buyContent.jsp?originNum=<%=originNum %>&rpageNum=<%=i%>&pageNum=<%=pageNum%>">[<%=i %>]</a>
		<%
				}//안쪽 for문
				//[다음] 끝페이지번호가 전체 페이지 개수보다 작을 때
				if(currentPage<pageCount){
		%>
					<a href="buyContent.jsp?originNum=<%=originNum %>&rpageNum=<%=currentPage+1%>&pageNum=<%=pageNum%>">[다음]</a>
		<%
				}
			}//바깥 if문
			%>
