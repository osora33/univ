<%@page import="ectripple.EctRippleDAO"%>
<%@page import="ectripple.EctRippleBean"%>
...
<%
	...
	EctBean ectBean = dao.getEct(originNum);
	
	int DBnum = ectBean.getNum();		//검색한 글번호
	int DBReadcount = ectBean.getReadcount();		//검색한 글의 조회수
	String DBName = ectBean.getName();		//검색한 글의 작성자명
	Timestamp DBDate = ectBean.getDate();		//검색한 글 작성일
	String DBSubject = ectBean.getSubject();		//검색한 글 제목
	String DBFileName = ectBean.getFile();
	...
	//-------------------------------덧글 DB작업-------------------------------------
	EctRippleDAO rippledao = new EctRippleDAO();
	EctRippleBean ripplebean = new EctRippleBean();
	...
	<div id="wrap">
		...
		<article>
			<table id="notice">
				...
				<tr>
					<td>첨부파일</td>
					<td colspan="3"><a href="../../reference/ect/download.jsp?fileName=<%=DBFileName%>"><%=DBFileName%></a></td>
				</tr>
				<tr class="contenttd">
					<td>글내용</td>
			...
			<%
				if(id != null){
			%>
			<form class = "rp" action="../../reference/ect/ripple.jsp" method="post">
...
<%-- 						<img src = "../images/board/level.gif" width = <%=wid%> height = "15"> --%>
			...
			%>
								...
								<script>
									function rdelete(a){
										...
										if(rdelete){
											location.href = "../../reference/ect/rdelete.jsp?pageNum=<%=pageNum%>&postnum=<%=bean.getPostnum()%>&num="+a;
										}
									}
								</script>
			<%
			...
			%>
					...
					<form action="../../reference/ect/rripple.jsp" method="post">
					...
					<form action="../../reference/ect/rupdate.jsp" method="post">
...
<%-- 						<a href = "freeboard.jsp?pageNum=<%=startPage-pageBlock %>">[이전]</a> --%>
					<a href = "ectContent.jsp?originNum=<%=originNum %>&rpageNum=<%=currentPage-1%>&pageNum=<%=pageNum%>">[이전]</a>
		<%
				}
				//[1][2]... 페이지 번호 나타내기
		...
		%>
					<a id= "a<%=i%>" href = "ectContent.jsp?originNum=<%=originNum %>&rpageNum=<%=i%>&pageNum=<%=pageNum%>">[<%=i %>]</a>
		<%
				}//안쪽 for문
				//[다음] 끝페이지번호가 전체 페이지 개수보다 작을 때
				if(currentPage<pageCount){
		%>
					<a href="ectContent.jsp?originNum=<%=originNum %>&rpageNum=<%=currentPage+1%>&pageNum=<%=pageNum%>">[다음]</a>
		<%
				}
			}//바깥 if문
			%>
