...
<%
	...
	ExamBean examBean = dao.getExam(originNum);
	...
	<div id="wrap">
		...
		<article>
			<table id="notice">
				...
				<tr>
					<td>첨부파일</td>
					<td colspan="3"><a href="../../reference/exam/load.jsp?fileName=<%=DBFileName%>"><%=DBFileName%></a></td>
				</tr>
				<tr class="contenttd">
					<td>글내용</td>
			...
			<%
				if(id != null){
			%>
			<form class = "rp" action="../../reference/exam/ripple.jsp" method="post">
...
<%-- 						<img src = "../images/board/level.gif" width = <%=wid%> height = "15"> --%>
			...
			%>
								...
								<script>
									function rdelete(a){
										...
										if(rdelete){
											location.href = "../../reference/exam/rdelete.jsp?pageNum=<%=pageNum%>&postnum=<%=bean.getPostnum()%>&num="+a;
										}
									}
								</script>
			<%
			...
			%>
					...
					<form action="../../reference/exam/rripple.jsp" method="post">
					...
					<form action="../../reference/exam/rupdate.jsp" method="post">
		...
		<script>
			function update(){
				...
				if(update){
					location.href = "../../reference/exam/update.jsp?num=<%=DBnum%>&pageNum=<%=pageNum%>";
				}
			}
			...
			function cdelete(){
				...
				if(cdelete){
					location.href="../../reference/exam/deletePro.jsp?num=<%=DBnum %>&pageNum=<%=pageNum %>"
				}
			}
		</script>
		<!-- 게시판 -->
...
<%-- 						<a href = "freeboard.jsp?pageNum=<%=startPage-pageBlock %>">[이전]</a> --%>
					<a href = "examContent.jsp?originNum=<%=originNum %>&rpageNum=<%=currentPage-1%>&pageNum=<%=pageNum%>">[이전]</a>
		<%
				}
				//[1][2]... 페이지 번호 나타내기
		...
		%>
					<a id= "a<%=i%>" href = "examContent.jsp?originNum=<%=originNum %>&rpageNum=<%=i%>&pageNum=<%=pageNum%>">[<%=i %>]</a>
		<%
				}//안쪽 for문
				//[다음] 끝페이지번호가 전체 페이지 개수보다 작을 때
				if(currentPage<pageCount){
		%>
					<a href="examContent.jsp?originNum=<%=originNum %>&rpageNum=<%=currentPage+1%>&pageNum=<%=pageNum%>">[다음]</a>
		<%
				}
			}//바깥 if문
			%>
