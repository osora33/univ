...
<%
	...
	BoardBean boardBean = dao.getBoard(originNum);
	...
	<div id="wrap">
		...
		<article>
			...
			<%
				if(id != null){
			%>
			<form class = "rp" action="../../board/freeboard/ripple.jsp" method="post">
...
// 					int rnum = bean.getNum();
			%>
				<form action="../../board/freeboard/rripple.jsp" method="post" name="fr">
					<tr>
					<%		
...
<%-- 						<img src = "../images/board/level.gif" width = <%=wid%> height = "15"> --%>
			...
			%>
								...
								<script>
									function rdelete(a){
										...
										if(rdelete){
											location.href = "../../board/freeboard/rdelete.jsp?pageNum=<%=pageNum%>&postnum=<%=bean.getPostnum()%>&num="+a;
										}
									}
								</script>
			<%
			...
			%>
					...
					<form action="../../board/freeboard/rupdate.jsp" method="post">
		...
		<script>
			function update(){
				...
				if(update){
					location.href = "../../board/freeboard/update.jsp?num=<%=DBnum%>&pageNum=<%=pageNum%>";
				}
			}
			...
			function cdelete(){
				...
				if(cdelete){
					location.href="../../board/freeboard/deletePro.jsp?num=<%=DBnum %>&pageNum=<%=pageNum %>"
				}
			}
		</script>
		<!-- 게시판 -->
		...
		<div id="page_control">
			...
			if(count > 0){
				...
				if(currentPage>1){
		%>
					<a href = "freeboardContent.jsp?originNum=<%=originNum %>&rpageNum=<%=currentPage-1%>&pageNum=<%=pageNum%>">[이전]</a>
		<%
				}
				//[1][2]... 페이지 번호 나타내기
		...
		%>
					<a id= "a<%=i%>" href = "freeboardContent.jsp?originNum=<%=originNum %>&rpageNum=<%=i%>&pageNum=<%=pageNum%>">[<%=i %>]</a>
		<%
				}//안쪽 for문
				//[다음] 끝페이지번호가 전체 페이지 개수보다 작을 때
				if(currentPage<pageCount){
		%>
					<a href="freeboardContent.jsp?originNum=<%=originNum %>&rpageNum=<%=currentPage+1%>&pageNum=<%=pageNum%>">[다음]</a>
		<%
				}
			}//바깥 if문
			%>
