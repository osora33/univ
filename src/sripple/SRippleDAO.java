...
public void insertRipple(SRippleBean rBean){
		...
		try {
			...
			sql = "select max(num) from sripple where postnum=?";
			...
			//DB에 저장된 글의 가장 최신 글번호 검색해 오는 SELECT문
			sql = "select max(pnum) from sripple";
			//참고 : DB에 글이 저장되어 있지 않는경우  새로추가할 글번호 는? 1
			//      DB에 글이 저장되어 있는 경우  검색한 가장 최신글번호 + 1 데이터를 새로추가할 글번호로 지정
			...
			sql = "insert into sripple(pnum, postnum, num, id, content, re_ref, re_lev, re_seq, date)"
				+ "values(?, ?, ?, ?, ?, ?, ?, ?, ?);";
	...
	public SRippleBean getRipple(int num){
		String sql = "";
		
		SRippleBean rBean = new SRippleBean();
		
		try{
			con = getConnection();
			sql = "select * from sripple where postnum = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	public List<SRippleBean> getRippleList(int startRow, int pageSize, int postnum){
		List<SRippleBean> rippleList = new ArrayList(); 
		String sql = "";
		...
		try{
			...
			sql = "select * from sripple where postnum=? order by re_ref, re_seq asc limit ?, ?";
	...
	public void reInsertRipple(SRippleBean rBean){
		String sql = "";
		...
		try{
			...
			sql = "select re_lev from sripple where postnum=? and num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, rBean.getPostnum());
			...
			sql = "select re_seq from sripple where postnum=? and num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, rBean.getPostnum());
			pstmt.setInt(2, rBean.getNum());
			rs = pstmt.executeQuery();
...
//			update 
			...
			//답변 달기
			//insert
			sql = "insert into sripple(pnum, postnum, num, id, content, Re_ref, re_lev, re_seq, date) values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, pnum);
			pstmt.setInt(2, rBean.getPostnum());
			pstmt.setInt(3, num);
	...
	//삭제할 글번호와 글을 삭제하기 위해 입력했던 비밀번호를 매개변수로 전달받아
		...
		public void deleteRipple(int postnum, int num){
			...
			try{
				con = getConnection();
				sql = "delete from sripple where postnum=? and num = ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, postnum);
				pstmt.setInt(2, num);
				pstmt.executeUpdate();
		...
		public void updateRipple(SRippleBean rBean){
			String sql = "";
			...
			try{
				...
				sql = "select * from sripple where num = ?";
				...
				if(rs.next()){
						sql = "update sripple set content=? where postnum=? and num=?";
						pstmt = con.prepareStatement(sql);
