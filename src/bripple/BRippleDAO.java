...
public void insertRipple(BRippleBean rBean){
		...
		try {
			...
			sql = "select max(num) from bripple where postnum=?";
			...
			//DB에 저장된 글의 가장 최신 글번호 검색해 오는 SELECT문
			sql = "select max(pnum) from bripple";
			//참고 : DB에 글이 저장되어 있지 않는경우  새로추가할 글번호 는? 1
			//      DB에 글이 저장되어 있는 경우  검색한 가장 최신글번호 + 1 데이터를 새로추가할 글번호로 지정
			...
			sql = "insert into bripple(pnum, postnum, num, id, content, re_ref, re_lev, re_seq, date)"
				+ "values(?, ?, ?, ?, ?, ?, ?, ?, ?);";
	...
	public BRippleBean getRipple(int num){
		String sql = "";
		
		BRippleBean rBean = new BRippleBean();
		
		try{
			con = getConnection();
			sql = "select * from bripple where postnum = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	public List<BRippleBean> getRippleList(int startRow, int pageSize, int postnum){
		List<BRippleBean> rippleList = new ArrayList(); 
		String sql = "";
		...
		try{
			...
			sql = "select * from bripple where postnum=? order by re_ref, re_seq asc limit ?, ?";
	...
	public void reInsertRipple(BRippleBean rBean){
		String sql = "";
		...
		try{
			...
			sql = "select re_lev from bripple where postnum=? and num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, rBean.getPostnum());
			...
			sql = "select re_seq from bripple where postnum=? and num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, rBean.getPostnum());
			pstmt.setInt(2, rBean.getNum());
			rs = pstmt.executeQuery();
...
//			sql = "update ripple set re_seq = re_seq+1 where re_ref = ? and re_seq>?";
//			pstmt = con.prepareStatement(sql);
//			pstmt.setInt(1, rBean.getRe_ref());		//주 글의 그룹번호
//			pstmt.setInt(2, rBean.getRe_seq());		//주 글의 글 입력순서
//			pstmt.executeUpdate();
...
//			update 
			...
			//답변 달기
			//insert
			sql = "insert into bripple(pnum, postnum, num, id, content, Re_ref, re_lev, re_seq, date) values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, pnum);
			pstmt.setInt(2, rBean.getPostnum());
			pstmt.setInt(3, num);
	...
	//삭제할 글번호와 글을 삭제하기 위해 입력했던 비밀번호를 매개변수로 전달받아
		...
		public void deleteRipple(int postnum, int num){
			...
			try{
				con = getConnection();
				sql = "delete from bripple where postnum=? and num = ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, postnum);
				pstmt.setInt(2, num);
				pstmt.executeUpdate();
				...
				System.out.println("deleteRipple메소드 내부에서 오류 : " + exception);
			}finally{
				자원해제();
			}
		}//deleteBoard 메소드 끝
		
		public void updateRipple(BRippleBean rBean){
			String sql = "";
			...
			try{
				...
				sql = "select * from bripple where num = ?";
				...
				if(rs.next()){
						sql = "update bripple set content=? where postnum=? and num=?";
						pstmt = con.prepareStatement(sql);
				...
				System.out.println("updateRipple 메소드 내부에서 오류 : " + exception);
			}finally{
				자원해제();
			}
		}
