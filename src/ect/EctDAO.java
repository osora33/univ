...
public class EctDAO {
	...
	//DB에 새글 추가 메소드
	public void insertEct(EctBean eBean){
		...
		try {
			...
			System.out.println("insertEct메소드 내부에서 실행 오류 : " + e);
		} finally {
			자원해제();
		}	
	}//insertBoard메소드 끝
	...
	public int getEctCount(){
		...
		try{
			...
			System.out.println("getEctCount메소드 내부에서 오류 : " + exception);
		}finally{
			자원해제();
		}
		return count;
	...
	public List<EctBean> getEctList(int startRow, int pageSize){
		List<EctBean> ectList = new ArrayList(); 
		String sql = "";
		...
		try{
			...
			sql = "select * from ect order by num desc limit ?, ?";
	...
	public void deleteEct(int num){
		...
		try{
			...
			sql = "delete from ect where num = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
			
			sql = "delete from ectripple where postnum = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
			...
			System.out.println("deleteEct메소드 내부에서 오류 : " + exception);
		}finally{
			자원해제();
		}
	}//deleteBoard 메소드 끝
	...
	public void updateEctF(EctBean eBean){
		String sql = "";
		...
		try{
			...
			if(rs.next()){
					sql = "update ect set subject=?, content=?, imgPath=?, file=? where num=?";
					pstmt = con.prepareStatement(sql);
			...
			System.out.println("updateEct 메소드 내부에서 오류 : " + exception);
		}finally{
			자원해제();
		}
	}
	
	public void updateEct(EctBean eBean){
		String sql = "";
		...
		try{
			...
			rs = pstmt.executeQuery();		//해당하는 글이 존재하면
			if(rs.next()){
					sql = "update ect set subject=?, content=?, imgPath=? where num=?";
					pstmt = con.prepareStatement(sql);
					pstmt.setString(1, eBean.getSubject());
					pstmt.setString(2, eBean.getContent());
					pstmt.setString(3, imgPath);
	...
	public int getSearchCount(String frselect, String search){
		...
		try{
			...
			if(frselect.equals("제목/내용")){
				sql = "select count(*) from ect where subject like ? or content like ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%"+search+"%");
				pstmt.setString(2, "%"+search+"%");
			}else if(frselect.equals("작성자")){
				sql = "select count(*) from ect where name like ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%"+search+"%");
			}
	...
	public List<EctBean> getSearchList(int startRow, int pageSize, String frselect, String search){
		List<EctBean> ectList = new ArrayList(); 
		String sql = "";
		...
		try{
			...
			if(frselect.equals("제목/내용")){
				sql = "select * from ect where subject like ? or content like ? limit ?, ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%"+search+"%");
				pstmt.setString(2, "%"+search+"%");
				...
				sql = "select * from ect where name like ? limit ?, ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%"+search+"%");
