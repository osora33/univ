...
public class BoardDAO {
	...
	public void insertBoard(BoardBean bBean){
		...
		try {
			...
			sql = "insert into board(num,name,"
	...
	public void deleteBoard(int num){
		...
		try{
			...
			sql = "delete from ripple where postnum = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
			...
			System.out.println("deleteBoard메소드 내부에서 오류 : " + exception);
		}finally{
			자원해제();
		}
	}//deleteBoard 메소드 끝
	...
	public void updateBoard(BoardBean bBean){
		...
		try{
			...
			if(rs.next()){
					sql = "update board set subject=?, content=?, imgPath=? where num=?";
					pstmt = con.prepareStatement(sql);
	...
	public void reInsertBoard(BoardBean bBean){
		...
		try{
			...
			//답변 달기
			//insert
			sql = "insert into board(num, name, subject, content, Re_ref, re_lev, re_seq, readcount, date, ip, imgPath) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			pstmt = con.prepareStatement(sql);
			...
			System.out.println("reInsertBoard 메소드 내부에서 오류 : " + exception);
		}finally{
			자원해제();
		}
	}
	...
	public List<BoardBean> getSearchList(int startRow, int pageSize, String frselect, String search){
		List<BoardBean> boardList = new ArrayList(); 
		String sql = "";
		...
		try{
			...
			if(frselect.equals("제목/내용")){
				sql = "select * from board where subject like ? or content like ? order by re_ref desc, re_seq asc limit ?, ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%"+search+"%");
				pstmt.setString(2, "%"+search+"%");
				...
				sql = "select * from board where name like ? order by re_ref desc, re_seq asc limit ?, ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%"+search+"%");
