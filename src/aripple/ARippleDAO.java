...
public void insertRipple(ARippleBean rBean){
		...
		try {
			...
			sql = "select max(num) from aripple where postnum=?";
			...
			//DB에 저장된 글의 가장 최신 글번호 검색해 오는 SELECT문
			sql = "select max(pnum) from aripple";
			//참고 : DB에 글이 저장되어 있지 않는경우  새로추가할 글번호 는? 1
			//      DB에 글이 저장되어 있는 경우  검색한 가장 최신글번호 + 1 데이터를 새로추가할 글번호로 지정
			...
			sql = "insert into aripple(pnum, postnum, num, id, content, re_ref, re_lev, re_seq, date)"
				+ "values(?, ?, ?, ?, ?, ?, ?, ?, ?);";
	...
	public ARippleBean getRipple(int num){
		String sql = "";
		
		ARippleBean rBean = new ARippleBean();
		
		try{
			con = getConnection();
			sql = "select * from aripple where postnum = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	public List<ARippleBean> getRippleList(int startRow, int pageSize, int postnum){
		List<ARippleBean> rippleList = new ArrayList(); 
		String sql = "";
		...
		try{
			...
			sql = "select * from aripple where postnum=? order by re_ref, re_seq asc limit ?, ?";
	...
	public void reInsertRipple(ARippleBean rBean){
		String sql = "";
		...
		try{
			...
			sql = "select re_lev from aripple where postnum=? and num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, rBean.getPostnum());
			...
			sql = "select re_seq from aripple where postnum=? and num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, rBean.getPostnum());
			pstmt.setInt(2, rBean.getNum());
			rs = pstmt.executeQuery();
...
//			update 
			...
			//답변 달기
			//insert
			sql = "insert into aripple(pnum, postnum, num, id, content, Re_ref, re_lev, re_seq, date) values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, pnum);
			pstmt.setInt(2, rBean.getPostnum());
			pstmt.setInt(3, num);
	...
	//삭제할 글번호와 글을 삭제하기 위해 입력했던 비밀번호를 매개변수로 전달받아
		...
		public void deleteRipple(int postnum, int num){
			...
			try{
				con = getConnection();
				sql = "delete from aripple where postnum=? and num = ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, postnum);
				pstmt.setInt(2, num);
				pstmt.executeUpdate();
		...
		public void updateRipple(ARippleBean rBean){
			String sql = "";
			...
			try{
				...
				sql = "select * from aripple where num = ?";
				...
				if(rs.next()){
						sql = "update aripple set content=? where postnum=? and num=?";
						pstmt = con.prepareStatement(sql);
