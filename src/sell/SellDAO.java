...
public class SellDAO {
	...
	//DataSource 커넥션풀을 얻고 커넥션풀 내부에 있는 Connection 객체를 얻는 메소드
		...
		public List<SellBean> getMainList(){
			List<SellBean> list = new ArrayList();
			String sql = null;
	...
	//DB에 새글 추가 메소드
	public void insertSell(SellBean sBean){
		...
		try {
			...
			sql = "insert into sell(num,name,"
				+ "subject,content,readcount,date,ip,imgPath,price,status)"
				+ "values(?,?,?,?,?,?,?,?,?,?);";
			
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);//추가할 새글의 글번호
			pstmt.setString(2, sBean.getName());//새글을 작성한 사람의 이름(id)
			pstmt.setString(3, sBean.getSubject());//추가할 새글의 제목
			pstmt.setString(4, sBean.getContent());//글내용
			pstmt.setInt(5, 0); //추가하는 새글의 조회수 0
			pstmt.setTimestamp(6, sBean.getDate()); //새글을 추가한 날짜 정보 
			pstmt.setString(7, sBean.getIp());//새글을 작성한 사람의  IP주소 정보 
			pstmt.setString(8, sBean.getImgPath());
			pstmt.setString(9, sBean.getPrice());
			pstmt.setString(10, sBean.getStatus());
	...
	public List<SellBean> getSellList(int startRow, int pageSize){
		List<SellBean> sellList = new ArrayList(); 
		String sql = "";
		...
		try{
			...
			sql = "select * from sell order by num desc limit ?, ?";
			...
			while(rs.next()){
				...
				gBean.setPrice(rs.getString("price"));
				gBean.setStatus(rs.getString("status"));
				
				//BoardBean객체 => ArrayList배열에 추가
				sellList.add(gBean);
			}	
		}catch(Exception exception){
			System.out.println("getSellList 메소드 내부에서 오류 : " + exception);
		}finally{
			자원해제();
		}
		return sellList;
	...
	public void updateReadCount(int num){
		...
		try{
			...
			sql = "select * from sell where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
			rs.next();
	...
	public SellBean getSell(int num){
		String sql = "";
		...
		try{
			...
			if(rs.next()){
				sBean.setContent(rs.getString("content"));
				sBean.setDate(rs.getTimestamp("date"));
				sBean.setIp(rs.getString("ip"));
				sBean.setName(rs.getString("name"));
				sBean.setNum(rs.getInt("num"));
				sBean.setReadcount(rs.getInt("readcount"));
				sBean.setSubject(rs.getString("subject"));
				sBean.setPrice(rs.getString("price"));
				sBean.setStatus(rs.getString("status"));
			}
		}catch(Exception exception){
			System.out.println("getSell메소드 내부에서 오류 : " + exception);
		}finally{
			자원해제();
		}
		return sBean;
	...
	public void deleteSell(int num){
		...
		try{
			...
			sql = "delete from sell where num = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
			
			sql = "delete from sripple where postnum = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
	...
	public void updateSell(SellBean sBean){
		String sql = "";
		...
		try{
			...
			if(rs.next()){
					sql = "update sell set subject=?, content=?, imgPath=?, price=?, status=? where num=?";
					pstmt = con.prepareStatement(sql);
					pstmt.setString(1, sBean.getSubject());
					pstmt.setString(2, sBean.getContent());
					pstmt.setString(3, sBean.getImgPath());
					pstmt.setString(4, sBean.getPrice());
					pstmt.setString(5, sBean.getStatus());
					pstmt.setInt(6, sBean.getNum());
					pstmt.executeUpdate();
			}
			
		}catch(Exception exception){
			System.out.println("updateBoard 메소드 내부에서 오류 : " + exception);
		}finally{
			자원해제();
		}
	}
	...
	public int getSearchCount(String frselect, String search){
		...
		try{
			...
			if(frselect.equals("제목/내용")){
				sql = "select count(*) from sell where subject like ? or content like ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%"+search+"%");
				pstmt.setString(2, "%"+search+"%");
			}else if(frselect.equals("작성자")){
				sql = "select count(*) from sell where name like ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%"+search+"%");
			}
	...
	public List<SellBean> getSearchList(int startRow, int pageSize, String frselect, String search){
		List<SellBean> sellList = new ArrayList(); 
		String sql = "";
		...
		try{
			...
			if(frselect.equals("제목/내용")){
				sql = "select * from sell where subject like ? or content like ? limit ?, ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%"+search+"%");
				pstmt.setString(2, "%"+search+"%");
				...
				sql = "select * from sell where name like ? limit ?, ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%"+search+"%");
			...
			while(rs.next()){
				...
				sBean.setReadcount(rs.getInt("readcount"));
				sBean.setSubject(rs.getString("subject"));
				sBean.setImgPath(rs.getString("imgPath"));
				sBean.setPrice(rs.getString("price"));
				sBean.setStatus(rs.getString("status"));
